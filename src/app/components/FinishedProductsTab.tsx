...
function SearchableMaterialSelect({
  ...
  return (
    <div className="relative mt-1" ref={dropdownRef}>
      <div
        className="w-full px-3 py-2 border border-gray-300 rounded-md bg-white cursor-pointer flex justify-between items-center text-sm min-h-[42px] hover:border-gray-400 focus:ring-2 focus:ring-secondary/20 transition-all"
        ...
        <div className="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-md shadow-xl overflow-hidden">
          ...
          <div className="max-h-48 overflow-y-auto py-1">
            {filteredMaterials.length === 0 ? (
              <div className="p-3 text-sm text-gray-500 text-center italic">No materials found.</div>
            ) : (
              filteredMaterials.map(material => (
                <div
                  key={material.id}
                  className={`px-3 py-2 text-sm cursor-pointer hover:bg-secondary/10 transition-colors ${
                    value === material.id ? 'bg-secondary/20 font-semibold text-primary' : 'text-gray-700'
                  }`}
                  onClick={() => {
                        ...
                        <div key={index} className="flex gap-2 items-end border p-3 rounded-lg bg-gray-50">
                          <div className="flex-1">
                            ...
                            <SearchableMaterialSelect
                              ...
                              onChange={(newMaterialId) => updateBOMMaterial(index, newMaterialId)}
                            />
                          </div>
                          <div className="w-32">
                    ...
                    <div key={index} className="flex gap-2 items-end border p-3 rounded-lg bg-gray-50">
                      <div className="flex-1">
                        ...
                        <SearchableMaterialSelect
                          materials={rawMaterials}
                          value={bom.materialId}
                          onChange={(newMaterialId) => updateEditBOMMaterial(index, newMaterialId)}
                        />
                      </div>
                      <div className="w-32">
                        <Label className="text-xs">Quantity</Label>
